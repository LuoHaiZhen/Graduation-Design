          ...
          <div class="goodsDetail">
            ...
            <div class="support">
              ...
              <div class="supportArea">
                ...
                <div class="fixWidth">{{ store.skuInfo.price }}</div>
              </div>
            </div>
          </div>
...
import Zoom from "./Zoom/theZoom.vue";
import TypeNav from "../../components/TypeNav/TypeNav.vue";
import { goodsStore } from "../../store/goods";
import { onMounted, provide, onBeforeMount, ref, toRefs, watch } from "vue";
...
watch(
  ...
  () => {
    ...
    if (skuNum.value < 1) {
      skuNum.value = 1;
      window.$message.error("购买数量不能为小于1");
    }
    //判断是否为不合法字符串
    if (isNaN(skuNum.value)) {
      skuNum.value = 1;
...
const changeActive = (saleAttrValue, arr) => {
  ...
  if (saleAttrValue.saleAttrName == info[0].saleAttrName) {
    info[0].saleAttrValueName = saleAttrValue.saleAttrValueName;
  } else {
    info[1].saleAttrValueName = saleAttrValue.saleAttrValueName;
  }
...
const changeSkuNum = (event) => {
  ...
  //不对num进行判断 直接赋值 在watch中判断
  skuNum.value = num;
};
//
const addShopcar = () => {
  const skuId = route.params.skuId;
  const Num = skuNum.value;
  console.log(skuId, Num);
  store.addShopCar(skuId, Num);
  sessionStorage.setItem("SKUINFO", JSON.stringify(store.skuInfo));
  store.info = info;
  //路由跳转
  router.push({
    name: "AddCartSuccess",
